...
public class HeartTransplant {
    ...
    public int addPerson(Person p, int arrayIndex) {
        ...
        if(arrayIndex <= this.listOfPatients.length - 1) {
    ...
    public int readPersonsFromFile(int numberOfLines) {
        ...
        for(int i = 0; i < numberOfLines; i++) {
            ...
            Person currPerson = new Person(id, ethnicity, gender, age, cause, urgency, state);
            addPerson(currPerson, i);
        }
    ...
    public int readSurvivabilityRateByAgeFromFile (int numberOfLines) {
        ...
        for (int i = 0; i < numberOfLines; i++) {
            int age = StdIn.readInt();
            int ypt = StdIn.readInt();
            double rate = StdIn.readDouble();

            SurvivabilityByAge sba = new SurvivabilityByAge(age, ypt, rate);
            this.survivabilityByAge[i] = sba;
        }
    ...
    public int readSurvivabilityRateByCauseFromFile (int numberOfLines) {
        ...
        for(int i = 0; i < numberOfLines; i++) {
            ...
            SurvivabilityByCause sbc = new SurvivabilityByCause(cause, ypt, rate);

            this.survivabilityByCause[i] = sbc;
        }
    ...
    public Person[] match(int numberOfHearts) {
        ...
        if(this.listOfPatients.length >= numberOfHearts)
